...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
#adding all the Observation variables in Observation variable list
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos  
            obsVars.append(obsVar)

#defining all edges in bayes net
...
#defining all values for each node variable of bayes net
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for eachRow in query.getAllPossibleAssignmentDicts():
            prob = query.getProbability(eachRow)
            if eachRow[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and eachRow[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += prob * WON_GAME_REWARD
                rightExpectedValue += prob * GHOST_COLLISION_REWARD
            elif eachRow[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and eachRow[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
