...
// 2103140 and 2103126
//Modifying policy utility calculation to incorporate -1 reward
#include <iostream>
#include <cstdlib>
#include <vector>
#include <cmath>
...
vector<double> policy_util(vector<int> state_policy)
{
    ...
    do
    {
        ...
        {
            new_util[i] = Prob_head*(-1 + gamma2*(old_util[i+state_policy[i]])) + (1-Prob_head)*(-1 + gamma2*(old_util[i-state_policy[i]]));
        }
    ...
    do{
        ...
        for(int j = 0; j < state_count; j++)
        {
            ...
            else
            {
                ...
                {
                    if(maxx < (Prob_head*(-1 + gamma2*old_util[i+j]) + (1-Prob_head)*(-1 +gamma2*old_util[j-i])) )
                    {
                        policy[j] = i;
                        maxx =(Prob_head*(-1 +gamma2*old_util[i+j]) + (1-Prob_head)*(-1 +gamma2*old_util[j-i]));
                        util[j] = maxx;
                    }
                }
            }
...
vector<int> policyOptimal_policyIteration()
{
    ...
    do{
        ...
        for(int j = 0; j < state_count; j++)
        {
                double maxx = policy_util(policy)[j];
                for(int i = 0; i <= min(j,N-j); i++)
                {                    
                    double eachAction_util = (Prob_head*(-1 + gamma2* policy_util(policy)[i+j]) + (1-Prob_head)*(-1 + gamma2* policy_util(policy)[j-i]));
